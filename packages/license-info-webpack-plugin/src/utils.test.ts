...
test("formatPackageInfo()", () => {
  ...
  expect(Object.keys(results).includes("removed")).toBeFalsy();
});

test("getPackageJson()", () => {
  const results = getPackageJson(`${__dirname}/../`);
  expect(results.name).toBe("license-info-webpack-plugin");
  expect(results.license).toBe("MIT");
});

test("getPackagePath()", () => {
  expect(getPackagePath("/dev/node_modules/foo/index.js")).toBe(
    "/dev/node_modules/foo"
  );
  expect(getPackagePath("/dev/node_modules/foo/lib/index.js")).toBe(
    "/dev/node_modules/foo"
  );
  // scoped modules
  expect(getPackagePath("/dev/node_modules/@user/foo/index.js")).toBe(
...
test("generateHtml() without license file", () => {
  const pkg = withoutLicensePkg;
  const results = generateHtml({ [`${pkg.name}@${pkg.version}`]: pkg });
  const expected = `
